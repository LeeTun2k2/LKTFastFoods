			...
			<div class="page-title-wrapper">
				<div class="page-title-heading">
					...
					<div>
						...
						<div class="page-title-subheading">Bạn có thể xem chi tiết
							thông tin từng cửa hàng cửa bạn tại đây</div>
					</div>
				</div>
			</div>
		</div>
		<form enctype="multipart/form-data"
			action="<c:url value="/vendor/listStore"/>" method="post">
								...
								<c:forEach items="${listStore}" var="x" varStatus="loop">
									...
									<tbody>
										<tr>
											...
											<td>${x.getName()}</td>
											<td>${x.getId()}</td>
											<td><c:if test="${x.isActive()}">Đang hoạt động</c:if> <c:if
													test="${!x.isActive()}">Không hoạt động</c:if></td>
											<td>${x.rating}*</td>
											<td>${x.getBio() }</td>
											<td><a
